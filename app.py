...
div[data-testid="stMarkdownContainer"] h3 { margin-top: 1rem; }
.bot-expander { margin-bottom: 8px; }
...
- 애매한 상황이면 반드시 "전문가 확인 필요"라고 명시하세요
- 근로기준법, 노동관련 법령 관점으로 검토하세요
- 리스크가 있으면 어떤 법 조항이 문제인지 구체적으로 언급하세요
- 핵심 포인트를 번호 목록으로 명확하게 제시하세요""",
        "short_system": "당신은 노무 전문 AI입니다. 다른 봇들의 의견을 보고 노무 관점에서 보완하거나 반박할 내용을 2-3문장으로만 간략히 의견을 내세요.",
    },
    {
        "name": "HRM봇",
        "emoji": "📋",
...
- 핵심 포인트를 번호 목록으로 명확하게 제시하세요""",
        "short_system": "당신은 HRM 전문 AI입니다. 다른 봇들의 의견을 보고 HRM/사내규정 관점에서 보완하거나 반박할 내용을 2-3문장으로만 간략히 의견을 내세요.",
    },
    {
        "name": "채용봇",
        "emoji": "🤝",
...
- 핵심 포인트를 번호 목록으로 명확하게 제시하세요""",
        "short_system": "당신은 채용 전문 AI입니다. 다른 봇들의 의견을 보고 채용 관점에서 보완하거나 반박할 내용을 2-3문장으로만 간략히 의견을 내세요.",
    },
    {
        "name": "계약봇",
        "emoji": "📝",
...
- 핵심 포인트를 번호 목록으로 명확하게 제시하세요""",
        "short_system": "당신은 계약 전문 AI입니다. 다른 봇들의 의견을 보고 계약 관점에서 보완하거나 반박할 내용을 2-3문장으로만 간략히 의견을 내세요.",
    },
    {
        "name": "총무봇",
        "emoji": "🏢",
...
- 효율적인 운영 방식을 제안하세요
- 핵심 포인트를 번호 목록으로 명확하게 제시하세요""",
        "short_system": "당신은 총무 전문 AI입니다. 다른 봇들의 의견을 보고 총무/운영 관점에서 보완하거나 반박할 내용을 2-3문장으로만 간략히 의견을 내세요.",
    },
    {
        "name": "급여봇",
        "emoji": "💰",
...
- 핵심 포인트를 번호 목록으로 명확하게 제시하세요""",
        "short_system": "당신은 급여/4대보험 전문 AI입니다. 다른 봇들의 의견을 보고 급여/보험 관점에서 보완하거나 반박할 내용을 2-3문장으로만 간략히 의견을 내세요.",
    },
    {
        "name": "교육봇",
        "emoji": "📚",
...
- 핵심 포인트를 번호 목록으로 명확하게 제시하세요""",
        "short_system": "당신은 교육/OJT 전문 AI입니다. 다른 봇들의 의견을 보고 교육/수습 관점에서 보완하거나 반박할 내용을 2-3문장으로만 간략히 의견을 내세요.",
    },
]
...
- 완벽한 답보다 현실적으로 실행 가능한 방향을 제시하세요"""

FINAL_SYSTEM = """당신은 웰파인 인사총무팀 AI 회의의 최종 정리 AI입니다.
성격: 냉철한 회의 진행자입니다.
역할: 모든 봇들의 토의 결과를 최종 정리합니다.
규칙:
...
with st.sidebar:
    ...
    # secrets에서 불러오기 (배포 시 자동 사용)
    api_key = api_key_input or st.secrets.get("ANTHROPIC_API_KEY", "") if hasattr(st, "secrets") else api_key_input

    st.markdown("**Claude 모델**")
    model_map = {
        "Sonnet (기본 권장)": "claude-sonnet-4-6",
        "Opus (고성능, 느림)": "claude-opus-4-8",
        "Haiku (빠름, 간단 작업용)": "claude-haiku-4-5-20251001",
    }
    selected_label = st.selectbox("모델 선택", list(model_map.keys()), label_visibility="collapsed")
    ...
    edited_prompt = st.text_area(
        "프롬프트",
        value=st.session_state.bot_prompts[bot_to_edit],
        height=200,
        label_visibility="collapsed"
    )
    ...
    st.divider()
    st.markdown("### 📝 누적 케이스")
    st.caption("쌓인 사례를 봇에 추가하면 웰파인 맞춤 답변이 나옵니다.")
    if "case_notes" not in st.session_state:
        st.session_state.case_notes = ""
    case_notes = st.text_area(
        "누적 케이스",
        value=st.session_state.case_notes,
        placeholder="예: 수습 불합격 처리 시 2회 이상 면담 기록 필요 (2025년 노무이슈 교훈)\nE-9 근로자 계약서는 반드시 이중언어 버전 사용...",
        height=150,
        label_visibility="collapsed"
    )
...
def get_relevant_bots(content: str, client: anthropic.Anthropic, model: str) -> list:
    try:
...
def call_bot(client, model, system, content, max_tokens=600) -> str:
    resp = client.messages.create(
        model=model,
...
def build_content(doc_text: str, question: str, case_notes: str) -> str:
    parts = []
    if case_notes.strip():
...
st.markdown('<div class="sub-title">문서나 질문을 올리면 전문 봇들이 검토하고 최적의 피드백을 드립니다 · 왼쪽 설정에서 API 키와 모델을 선택하세요</div>', unsafe_allow_html=True)
...
if start:
    ...
    with st.spinner("🔍 회의 준비 중..."):
        relevant = get_relevant_bots(full_content, client, selected_model)
    ...
    for bot in BOTS:
        ...
        short_system = bot["short_system"]
        use_system = system if is_core else short_system
        ...
        with placeholder:
            with st.spinner(f'{bot["emoji"]} {bot["name"]} 검토 중...'):
                try:
                    result = call_bot(client, selected_model, use_system, full_content, max_tok)
    ...
    factcheck_input = full_content + "\n\n[전문 봇들의 의견]\n" + "\n".join(
        f"\n{name}:\n{opinion}" for name, opinion in bot_results.items()
    )
    factcheck_system = st.session_state.bot_prompts.get("팩트체크봇", FACTCHECK_SYSTEM)
    with st.spinner("🔍 팩트체크봇 검증 중..."):
        try:
            factcheck_result = call_bot(client, selected_model, factcheck_system, factcheck_input, 900)
        except Exception as e:
            factcheck_result = f"오류: {e}"
    with st.expander("🔍 **팩트체크봇**", expanded=True):
        st.markdown(factcheck_result)

    # ── 3라운드: 실무봇 ──
    st.markdown("### 3️⃣ 실무 검토")
    field_input = factcheck_input + f"\n\n팩트체크봇:\n{factcheck_result}"
    field_system = st.session_state.bot_prompts.get("실무봇", FIELD_SYSTEM)
    with st.spinner("🔧 실무봇 검토 중..."):
        try:
            field_result = call_bot(client, selected_model, field_system, field_input, 700)
        except Exception as e:
            field_result = f"오류: {e}"
    ...
    final_system = st.session_state.bot_prompts.get("최종정리봇", FINAL_SYSTEM)
    with st.spinner("🎯 최종정리봇 정리 중..."):
        try:
            final_result = call_bot(client, selected_model, final_system, final_input, 1500)
        except Exception as e:
            final_result = f"오류: {e}"
